...
xgb_eval <- soft_simputes %>% 
  select(lambda, xgb_fit) %>% 
  deframe() %>% 
  map_dfr('evaluation_log', .id = 'lambda') %>%
  mutate(
    lambda = as.numeric(lambda),
    lamgrp = as.factor(lambda)
  ) %>% 
  ggplot(aes(x=iter, y=test_auc, col=lambda, group = lamgrp)) +
  geom_line()


mi_num <- 10

mi_prd <- soft_simputes %>% 
...
soft_simputes %>% 
  ...
  select(impute, auc = test_auc) %>% 
  mutate(impute = paste("SI, iter", impute)) %>% 
